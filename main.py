...
def display_ascii_art(frame):
    ...
    if new_height > term_size.lines:
        new_height = term_size.lines
        new_width = int(new_height * width * 2 / height)
    resized_frame = cv2.resize(gray_frame, (new_width, new_height))

    colored_ascii_art = np.empty_like(resized_frame, dtype=object)
    for y in range(new_height):
        for x in range(new_width):
            ...
            colored_ascii_art[y, x] = f"{ascii_color}{char}{Style.RESET_ALL}"

    ascii_art = '\n'.join([''.join(row) for row in colored_ascii_art])
    os.system('cls' if os.name == 'nt' else 'clear')  # Use 'cls' on Windows and 'clear' on Linux/macOS
